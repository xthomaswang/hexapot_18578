    ...
    while (1) {
        ...
        if (ch == '\n' || ch == '\r') {
            ...
            for (int i = 0; i < NUM_SERVOS; ++i) {
                float a = angle_f[i];
                if (a < (float)SERVO_MIN_DEG) a = (float)SERVO_MIN_DEG;
                if (a > (float)SERVO_MAX_DEG) a = (float)SERVO_MAX_DEG;
                angle_deg[i] = (uint16_t)(a + 0.5f);
            }
...
void app_main(void)
{
    ...
    /* No initial servo pose: PCA9685 channels stay off until the Pi sends angles. */

    xTaskCreate(pi_uart_rx_task, "pi_uart_rx_task", 4096, NULL, 5, NULL);
    xTaskCreate(imu_debug_task,  "imu_debug_task",  4096, NULL, 4, NULL);

    ESP_LOGI(TAG, "Ready: waiting for Pi UART angles; dual PCA9685 + BNO055 initialized");
}
