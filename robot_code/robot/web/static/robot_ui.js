(function () {
  ...
  function calibrationDefaultAnglesForLeg(calibrationState, legIndex) {
    const frame = calibrationState && Array.isArray(calibrationState.default_frame) ? calibrationState.default_frame : null;
    if (frame && frame.length >= (legIndex + 1) * 3) {
      return {
        coxa_deg: Number(frame[legIndex * 3] || 0),
        femur_deg: Number(frame[legIndex * 3 + 1] || 0),
        tibia_deg: Number(frame[legIndex * 3 + 2] || 0),
      };
    }
    if (calibrationState && calibrationState.baseline_angles_deg &&
        Number(calibrationState.config && calibrationState.config.leg_index) === legIndex) {
      const bl = calibrationState.baseline_angles_deg;
      return {
        coxa_deg: Number(bl.coxa_deg || 0),
        femur_deg: Number(bl.femur_deg || 0),
        tibia_deg: Number(bl.tibia_deg || 0),
      };
    }
    return { coxa_deg: 90, femur_deg: 64.23419122714509, tibia_deg: 116.33185549681369 };
  }

  function calibrationSavedCumulativeDelta(calibrationState, legIndex) {
    const entry = calibrationLegEntry(calibrationState, legIndex);
    const baseline = calibrationDefaultAnglesForLeg(calibrationState, legIndex);
    if (!entry || !entry.saved || !entry.angles_deg) {
      return { coxa_deg: 0, femur_deg: 0, tibia_deg: 0 };
    }
    const saved = entry.angles_deg;
    return {
      coxa_deg: Number(saved.coxa_deg || 0) - baseline.coxa_deg,
      femur_deg: Number(saved.femur_deg || 0) - baseline.femur_deg,
      tibia_deg: Number(saved.tibia_deg || 0) - baseline.tibia_deg,
    };
  }

  function calibrationDraftForLeg(calibrationState, legIndex) {
    const savedCum = calibrationSavedCumulativeDelta(calibrationState, legIndex);
    const config = calibrationState && calibrationState.config;
    if (config && Number(config.leg_index || 0) === legIndex) {
      return {
        coxa_deg: savedCum.coxa_deg + Number(config.coxa_deg || 0),
        femur_deg: savedCum.femur_deg + Number(config.femur_deg || 0),
        tibia_deg: savedCum.tibia_deg + Number(config.tibia_deg || 0),
      };
    }
    return savedCum;
  }
  ...
  function clampAngle(value) {
    return clampNumber(Number(value), 0, 180);
  }
  ...
  function readInputString(id, fallback) {
    const node = el(id);
    if (!node) {
      return fallback;
    }
  ...
  function readInputNumber(id, fallback, options) {
    const opts = options || {};
    const node = el(id);
    if (!node) {
      return fallback;
  ...
  function runtimePayload() {
    return {
      port: readInputString("runtime-port", defaults.runtime.port),
      baudrate: readInputNumber("runtime-baudrate", defaults.runtime.baudrate, { positive: true }),
      verbose: el("runtime-verbose") ? el("runtime-verbose").checked : defaults.runtime.verbose,
      deadzone: readInputNumber("runtime-deadzone", defaults.runtime.deadzone),
      stride: readInputNumber("runtime-stride", defaults.runtime.stride),
      lift_scale: readInputNumber("runtime-lift-scale", defaults.runtime.lift_scale),
      frequency: readInputNumber("runtime-frequency", defaults.runtime.frequency),
      yaw_stride_deg: readInputNumber("runtime-yaw-stride-deg", defaults.runtime.yaw_stride_deg),
      stance_z: readInputNumber("runtime-stance-z", defaults.runtime.stance_z),
  ...
  function calibrationPayload() {
    ...
    const savedCum = calibrationSavedCumulativeDelta(currentCalibrationState(), calibrationSelectedLeg);
    return {
      port: readInputString("runtime-port", runtimeDefaults.port || "/dev/serial0"),
      baudrate: readInputNumber("runtime-baudrate", runtimeDefaults.baudrate || 115200, { positive: true }),
      leg_index: calibrationSelectedLeg,
      coxa_deg: angles.coxa_deg - savedCum.coxa_deg,
      femur_deg: angles.femur_deg - savedCum.femur_deg,
      tibia_deg: angles.tibia_deg - savedCum.tibia_deg,
    };
  }
  ...
  function renderCalibrationLegSelection(calibration) {
    ...
    setText(
      ...
      selectedSaved
        ? "Saved neutral pose loaded. Sliders show cumulative offset from default \u2014 drag further to refine, or Zero Draft to snap back to saved."
  ...
  async function resetCalibrationProfile() {
    if (typeof window !== "undefined" && window.confirm && !window.confirm("Delete the calibration profile file and reset all saved legs to defaults?")) {
      return;
    }
    await runAction(async function () {
      const result = await postJSON("/api/calibration/reset", {}, false, ACTION_TIMEOUT_MS);
      applyStatus(result.data);
      syncCalibrationEditorFromStatus(result.data);
      setCalibrationSliderValues(calibrationDefaults());
      showToast("Calibration profile reset", "info");
    }, ACTION_TIMEOUT_MS);
  }
  ...
  function bindCalibrationControls() {
    ...
    const resetButton = el("calibration-reset");
    ...
    if (neutralButton) {
      neutralButton.addEventListener("click", function () {
        setCalibrationSliderValues(calibrationSavedCumulativeDelta(currentCalibrationState(), calibrationSelectedLeg));
        updateCommandPreviews();
        sendCalibrationPose(true);
      });
    }
